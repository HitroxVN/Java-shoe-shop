...
<body>
 <h2>Sửa sản phẩm</h2>
        ...
        <form action="ProductController" method="post" enctype="multipart/form-data">
            ...
            <c:set var="selectedSizes" value="${fn:split(product.size, ',')}" />
			        ...
			        <c:forEach var="sz" begin="36" end="45">
			            <label style="margin-right: 10px;">
			                <input type="checkbox" 
			                       name="sizes" 
			                       value="${sz}"
			                       <c:if test="${fn:contains(product.size, sz)}">checked</c:if>>
			                ${sz}
			            </label>
			        </c:forEach>
			    </div>
            ...
            <div class="form-group">
                <label for="image">Ảnh sản phẩm mới (để trống nếu không đổi)</label>
